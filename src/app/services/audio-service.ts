...
export class AudioService {
     ...
     * @param src 
     */
    public setAudio(src: string, playAfter: boolean = true): void {
      this.audio.src = src;
      if (playAfter) this.playAudio();
      setTimeout(() => {
        let seconds = Math.floor(this.audio.duration % 60),
          displaySecs = (seconds < 10) ? '0' + seconds : seconds,
          minutes = Math.floor((this.audio.duration / 60) % 60),
          displayMins = (minutes < 10) ? '0' + minutes : minutes;
        this.duration = displayMins ? displayMins + ':' + displaySecs : '--:--'
      }, 250)
    }
  ...
  public isPlaying(): boolean {
    return this.playerStatus.getValue() == 'playing'
  }
}
